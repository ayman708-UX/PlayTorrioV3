...
nif not exist "%RELEASE_DIR%\playtorrio.exe" (
  echo playtorrio.exe not found in %RELEASE_DIR%
  exit /b 1
)
necho Running playtorrio.exe from %RELEASE_DIR%
pushd "%RELEASE_DIR%"nn:: Run and capture output to log (PowerShell redirection to capture console output from GUI apps)
